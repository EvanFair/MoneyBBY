...
function initFeedCuration() {
    ...
    if (filterSearch) filterSearch.addEventListener("input", filterStories);
    if (filterSource) filterSource.addEventListener("change", filterStories);
    if (filterCategory) filterCategory.addEventListener("change", filterStories);
    if (filterRating) filterRating.addEventListener("change", filterStories);
...
function loadCurationStories() {
    ...
    Promise.all([
        ...
        populateSourceDropdown(scraped); // Populate the dynamic source list
        filterStories(); // Filter and render
...
function populateSourceDropdown(scrapedStories) {
    const select = document.getElementById("filter-source");
    if (!select) return;

    // Get unique sources
    const sources = [...new Set(scrapedStories.map(s => s.source).filter(Boolean))].sort();
    
    // Remember current selection
    const currentSelection = select.value;

    let html = `<option value="">All Sources</option>`;
    html += sources.map(src => `<option value="${src}">${src}</option>`).join("");
...
function filterStories() {
    const container = document.getElementById("story-list-container");
    if (!container || !window.loadedStories) return;
    ...
    const filtered = window.loadedStories.filter(story => {
        // Search filter
        if (query) {
            const title = (story.title || "").toLowerCase();
            const summary = (story.summary || "").toLowerCase();
            const cleanSummary = (story.clean_summary || "").toLowerCase();
            
            // Check tags (stored as JSON string or plain CSV)
            let tagMatch = false;
            if (story.niche_tags) {
                tagMatch = story.niche_tags.toLowerCase().includes(query);
            }
            
            const match = title.includes(query) || 
                          summary.includes(query) || 
                          cleanSummary.includes(query) ||
                          tagMatch;
        ...
        // Rating (Value Score) filter
        if (rating) {
            const score = story.value_score != null ? parseFloat(story.value_score) : 0;
            if (rating === "high" && score < 7) return false;
            if (rating === "medium" && score < 5) return false;
            if (rating === "low" && score >= 5) return false;
        }

        return true;
    });

    // Update the displayed stat scraped count
    const statScraped = document.getElementById("stat-scraped");
    if (statScraped) {
        statScraped.textContent = filtered.length === window.loadedStories.length 
            ? window.loadedStories.length 
            : `${filtered.length} of ${window.loadedStories.length}`;
    }
    ...
    if (filtered.length === 0) {
        container.innerHTML = `
            <div class="detail-placeholder">
                ...
                <p>Try adjusting your search filters to show more pending stories.</p>
            </div>`;
    } else {
        container.innerHTML = filtered.map(story => renderStoryCard(story)).join("");
    }
}
...
function initPipeline() {
    ...
    if (_pipelineRefreshTimer) clearInterval(_pipelineRefreshTimer);
    _pipelineRefreshTimer = setInterval(() => {
        loadPipelineStatus();
        loadPipelineLog();
    }, 30000);
}
...
function loadPipelineStatus() {
    fetch(`${API_BASE}/api/pipeline/status`)
        .then(r => r.json())
        .then(data => {
            document.getElementById('pl-scraped').textContent   = data.stories_scraped    ?? '—';
            document.getElementById('pl-approved').textContent  = data.stories_approved   ?? '—';
            document.getElementById('pl-carousels').textContent = data.carousels_generated ?? '—';
            document.getElementById('pl-reels').textContent     = data.reels_generated    ?? '—';
            document.getElementById('pl-published').textContent = data.posts_published    ?? '—';
            ...
            if (data.running) {
                ...
                const lastRun  = data.last_run
                    ...
                    : 'Never';
                text.textContent = `Idle — last run: ${lastRun}`;
            }
        })
        .catch(() => {
            document.getElementById('pipeline-status-text').textContent = 'Could not reach backend.';
        });
}

function loadPipelineLog() {
    fetch(`${API_BASE}/api/pipeline/log?lines=80`)
        .then(r => r.json())
        .then(data => {
            const box = document.getElementById('pipeline-log-box');
            if (!box) return;
            if (!data.lines || data.lines.length === 0) {
                box.innerHTML = '<div style="color:var(--text-muted);padding:16px;">No pipeline runs yet. Click "Run" to start.</div>';
                return;
            }
            box.innerHTML = data.lines.map(line => {
                let cls = 'log-line';
                if (line.includes('ERROR') || line.includes('✗'))  cls += ' log-error';
                else if (line.includes('✓') || line.includes('complete') || line.includes('Done')) cls += ' log-success';
                else if (line.includes('STEP') || line.includes('===')) cls += ' log-header';
...
window.runPipeline = function(withPublish) {
    const btn = document.getElementById(withPublish ? 'btn-pipeline-publish' : 'btn-pipeline-no-publish');
    if (btn) { btn.disabled = true; btn.textContent = '⏳ Starting...'; }

    fetch(`${API_BASE}/api/pipeline/run`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ top_stories: 5, publish: withPublish, skip_carousel: false, skip_reel: false })
    })
    .then(r => r.json())
    .then(data => {
        showToast(data.message || 'Pipeline started.');
        setTimeout(() => { loadPipelineStatus(); loadPipelineLog(); }, 2000);
        setTimeout(() => { loadPipelineStatus(); loadPipelineLog(); }, 10000);
    })
    .catch(err => showToast('Pipeline start failed: ' + err))
    .finally(() => {
        if (btn) { btn.disabled = false; btn.textContent = withPublish ? '🚀 Run + Publish' : '🔄 Run (No Publish)'; }
    });
};
...
// ─── Distribute Tab — real publish buttons ────────────────────────────────────

window.publishCarousel = function(storyId, btnEl) {
    if (btnEl) { btnEl.disabled = true; btnEl.textContent = '⏳'; }
    fetch(`${API_BASE}/api/publish/carousel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ story_id: storyId, platforms: ['instagram', 'facebook'] })
    })
    .then(r => r.json())
    .then(data => {
        const results = data.results || [];
        const ok = results.filter(r => r.success).map(r => r.platform);
        const fail = results.filter(r => !r.success).map(r => r.platform);
        if (ok.length)   showToast(`✓ Carousel posted to: ${ok.join(', ')}`);
        if (fail.length) showToast(`✗ Failed on: ${fail.join(', ')}`);
        if (btnEl) { btnEl.disabled = false; btnEl.textContent = ok.length ? '✓ Posted' : '✗ Failed'; }
    })
    .catch(err => {
        showToast('Publish error: ' + err);
        if (btnEl) { btnEl.disabled = false; btnEl.textContent = '📤 Post Carousel'; }
    });
};

window.publishReel = function(episodeId, btnEl) {
    if (btnEl) { btnEl.disabled = true; btnEl.textContent = '⏳'; }
    fetch(`${API_BASE}/api/publish/reel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ episode_id: episodeId, platforms: ['instagram', 'facebook'] })
    })
    .then(r => r.json())
    .then(data => {
        const results = data.results || [];
        const ok = results.filter(r => r.success).map(r => r.platform);
        const fail = results.filter(r => !r.success).map(r => r.platform);
        if (ok.length)   showToast(`✓ Reel posted to: ${ok.join(', ')}`);
        if (fail.length) showToast(`✗ Failed on: ${fail.join(', ')}`);
        if (btnEl) { btnEl.disabled = false; btnEl.textContent = ok.length ? '✓ Posted' : '✗ Failed'; }
    })
    .catch(err => {
        showToast('Publish error: ' + err);
        if (btnEl) { btnEl.disabled = false; btnEl.textContent = '🎬 Post Reel'; }
    });
};

window.generateCarousel = function(storyId, btnEl) {
    if (btnEl) { btnEl.disabled = true; btnEl.textContent = '⏳ Generating...'; }
    fetch(`${API_BASE}/api/stories/${storyId}/carousel`, { method: 'POST' })
        .then(r => r.json())
        .then(() => {
            showToast(`Carousel generating for story #${storyId}...`);
            if (btnEl) { btnEl.disabled = false; btnEl.textContent = '🖼 Re-generate'; }
        })
        .catch(() => { if (btnEl) { btnEl.disabled = false; btnEl.textContent = '🖼 Carousel'; } });
};

window.generateReel = function(storyId, btnEl) {
    if (btnEl) { btnEl.disabled = true; btnEl.textContent = '⏳ Generating...'; }
    fetch(`${API_BASE}/api/stories/${storyId}/reel`, { method: 'POST' })
        .then(r => r.json())
        .then(() => {
            showToast(`Reel generating for story #${storyId}...`);
            if (btnEl) { btnEl.disabled = false; btnEl.textContent = '🎬 Re-generate'; }
        })
        .catch(() => { if (btnEl) { btnEl.disabled = false; btnEl.textContent = '🎬 Reel'; } });
};

window.autoApproveStories = function(btnEl) {
    if (btnEl) { btnEl.disabled = true; btnEl.textContent = '⏳ Approving...'; }
    fetch(`${API_BASE}/api/auto-approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(r => r.json())
    .then(data => {
        showToast(data.message || 'Auto-approved stories.');
        loadCurationStories();
        if (btnEl) { btnEl.disabled = false; btnEl.textContent = '⚡ Auto-Approve Top 5'; }
    })
    .catch(() => { if (btnEl) { btnEl.disabled = false; btnEl.textContent = '⚡ Auto-Approve Top 5'; } });
};
